...
//go:embed pathlist.txt
var pathListData string

// loadPathList 从嵌入的 pathlist.txt 解析路径列表，首行空行保留为 "" 代表根路径
func loadPathList() []string {
	var paths []string
	for _, line := range strings.Split(pathListData, "\n") {
		line = strings.TrimRight(line, "\r")
		paths = append(paths, line)
	}
	// 去掉末尾可能多出的空行（文件最后一行换行导致）
...
func pushTarget(expr string) {
	...
	if uri.IsIPv4(expr) {
		// 这是默认扫描端口的行为
		//fmt.Printf("走了这里扫端口的 默认会指定一些高频端口的\n")
	...
	if uri.IsNetlocPort(expr) {
		//netloc, _ := uri.SplitWithNetlocPort(expr)
	...
	if uri.IsURL(expr) {
		...
		if u != nil {
			...
			// 如果解析出来的 URL 是空的，打印提示并返回，防止崩溃
			//fmt.Printf("[!] 跳过无法解析的URL: %s\n", expr)
			return
		}
		if app.Setting.Check == false {
...
var (
	// 使用 URL 字符串作为 Key，确保同一个 URL 只进入队列一次
	urlFilter sync.Map
)

func pushURLTarget(URL *url.URL, response *gonmap.Response) {
	// 1. 基础去重逻辑
	cleanURL := strings.TrimSuffix(URL.String(), "/")
	if _, loaded := urlFilter.LoadOrStore(cleanURL, true); loaded {
		// 如果已经存在，直接跳过，不再向下分发
		return
	}
	var cli *http.Client
...
func match_finger(banner appfinger.Banner, statusCode int) scanResult {

	body := banner.Body
	// 这里是大写的 headers 区分大小写的，写指纹的时候要注意
	headers := banner.Header
	icon := banner.Icon
	...
	for _, f := range finger.Fingerprints {
		if len(f.Keyword) == 0 {
			continue
		}
		switch f.Method {
	...
	r := scanResult{
		...
		Matches: strings.Join(removeDuplicate(matches), ";"),
		BodyLen: len(body),
	}
	//matchedServices := strings.Join(r.Matches, ",")
	//if len(r.Matches) > 0 {
	//	fmt.Printf("r:\n titile: %s | match: %s\n", r.Title, matchedServices)
	//}
...
func generateURLScanner(wg *sync.WaitGroup) *scanner.URLClient {
	...
	client.HandlerMatched = func(URL *url.URL, final_url *url.URL, banner *appfinger.Banner, finger *appfinger.FingerPrint, statusCode int) {
		//fmt.Printf("==这里是appfinger =\n%s %s\n", URL, strings.Join(finger.ProductName, ","))
		r := match_finger(*banner, statusCode)

		//addr, _ := cdn.Find(URL.Hostname())
		// 漏扫的逻辑
		if len(r.Matches) < 200 && statusCode == 200 {
			//fmt.Printf("%-40s 🔴 %s \n", final_url, r.Matches)
			slog.Println(slog.DATA, fmt.Sprintf("%-40s 🤓 %s  |  %s", final_url, r.Matches, r.Status))
		}
		if r.Matches != "" && len(r.Matches) < 150 && statusCode == 200 {
			tags := strings.Split(r.Matches, ";")
			//fmt.Printf("[vuln-scan] %s Tags: %v\n", URL, tags)
			urlStr := URL.String()
			nlscan.Nucleiscan([]string{urlStr}, tags)
		}
...
func outputHydraSuccess(addr net.IP, port int, protocol string, auth *hydra.Auth) {
	...
	m["username"] = user
	m["password"] = pwd
	//Addr, _ := cdn.Find(addr.String())
	URL, _ := url.Parse(target)
	//fmt.Printf("%s\tcrackSuccess\tProductName:%s/%s,Addr:%s\n", URL, user, pwd, Addr)
	//fmt.Printf("%s\tcrackSuccess\t%s/%s\n", URL, user, pwd)
	slog.Println(slog.DATA, fmt.Sprintf("%s\t%s\t%s/%s\n", URL, color.RedB("crackSuccess"), user, pwd))
...
func outputOpenResponse(addr net.IP, port int) {
	...
	URL, _ := url.Parse(target)
	outputHandler(URL, "", map[string]string{
		"IP":   URL.Hostname(),
		"Port": strconv.Itoa(port),
	})
}
...
func outputHandler(URL *url.URL, keyword string, m map[string]string) {
	...
	printStr := ""
	if keyword == "" {
		printStr = fmt.Sprintf(format, URL.String(), "NULL", fingerPrint)
	} else {
		printStr = fmt.Sprintf(format, URL.String(), keyword, fingerPrint)
	}
	//printStr := fmt.Sprintf("%-30v %v %v", URL.String(), keyword, fingerPrint)
