...
func ConfigInit() {
	...
	//quake模块
	Setting.Quake = args.quake
	Setting.QuakeSize = args.quakeSize
...
// 这里是 端口的选择， 优先试用用户给出的 然后是 --top端口切片， 最后是 从1000里面取前400个
func (c *Config) loadPort() {
	...
	//fmt.Println("qweqwe 参数为：===", len(c.Port))
	//fmt.Println("qweqwe 参数为：===", c.Port)
