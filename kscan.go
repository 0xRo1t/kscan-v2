...
options:
  -c         针对目标地址做指纹识别，仅不会进行端口探测
  --scan          将针对--fofa、--spy提供的目标对象，进行端口扫描和指纹识别，fofa资产跑redis这种弱口令加这个参数
...
fofa options:
   -ff  将获取fofa搜索语法说明
   -fs   将设置fofa返回条目数，默认100条
   --fofa-fix-keyword 修饰keyword，该参数中的{}最终会替换成-f参数的值
`

const usage = `[+] ./kscan [-h,-ff] [-q -qs][-t,-f,--spy] 
[+] ./kscan -t ip -p 1-65535 -hydra 扫描端口、指纹、爆破弱口令
[+] fofa搜索，然后用我们的指纹跑一遍 顺便探测存活:⬇️
[+] ./kscan -f domain="qq.com" -c 
[quake] 语法如下
...
owner	IP 归属单位	主机数据、服务数据	这里的归属并不精确，	owner: "tencent.com" owner: "清华大学"
...
func main() {
	...
	//quake模块初始化
	if len(app.Setting.Quake) > 0 {
		Init_quake()
		query := app.Setting.Quake[0]
		size := app.Setting.QuakeSize
		quake.Run(query, size)
		if app.Setting.Check == false && app.Setting.Scan == false {
			slog.Println(slog.WARN, "可以使用-c参数对quake扫描结果进行存活性及指纹探测,也可以使用--scan参数对quake扫描结果进行端口扫描")
		}
		if app.Setting.Check == true {
			app.Setting.Target = quake.GetUrlTarget()
			slog.Println(slog.WARN, "check参数已启用，现在将对quake扫描结果进行存活性及指纹探测")
			fmt.Println("====================================================================================")
			fmt.Println("====================================================================================")
		}
		if app.Setting.Scan == true {
			app.Setting.Target = quake.GetHostTarget()
			slog.Println(slog.WARN, "scan参数已启用，现在将对quake扫描结果进行端口扫描及指纹探测")
		}
	}
	...
	if len(app.Setting.Fofa) > 0 {
		...
		fofa.Run()
		if app.Setting.Check == false && app.Setting.Scan == false {
			slog.Println(slog.WARN, "可以使用-c参数对fofa扫描结果进行存活性及指纹探测，也可以使用--scan参数对fofa扫描结果进行端口扫描")
		}
		if app.Setting.Check == true {
...
var finger_json embed.FS

const (
	qqwryPath = "static/qqwry.dat"
)

func InitKscan() {
	//HTTP指纹库初始化
	finger.SetEmbeddedFS(finger_json)
	finger.LoadFingerJSON("finger.json")
	//fmt.Printf("[+] 加载 %d 条指纹\n", len(finger.Fingerprints))
